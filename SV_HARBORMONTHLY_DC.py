...
import os
import shutil
os.chdir('C:\\Users\\alejandro.gutierrez\\OneDrive - Carlin Group - CA Fortune\\Documents\\ALEJANDRO RAMOS GTZ\\PYTHON') # relative path: scripts dir is under Lab
...
class Download_SuperValue_Data(unittest.TestCase):
    ...
    def setUp(self):
      ...
      #  chrome_options.add_argument('--headless')
        ...
        #Set export paths
        self.export_Monthly_DC = 'C:\\Users\\alejandro.gutierrez\\OneDrive - Carlin Group - CA Fortune\\Documents\\KROGER SELENIUM\\SUPER VALU\\MONTHLY_DC'
        self.export_Monthly_VOID = 'C:\\Users\\alejandro.gutierrez\\OneDrive - Carlin Group - CA Fortune\\Documents\\KROGER SELENIUM\\SUPER VALU\\VOID'
        self.export_Melitta = 'C:\\Users\\alejandro.gutierrez\\OneDrive - Carlin Group - CA Fortune\\Documents\\KROGER SELENIUM\\SUPER VALU\\MELITTA'
        
        #Set paths for do the macros 
        self.import_path = 'C:\\Users\\alejandro.gutierrez\\OneDrive - Carlin Group - CA Fortune\\Documents\\KROGER SELENIUM\\SUPER VALU\\MONTHLY_DC'
        self.export_path = 'C:\\Users\\alejandro.gutierrez\\OneDrive - Carlin Group - CA Fortune\\Documents\\KROGER SELENIUM\\SUPER VALU\\MONTHLY_DC\\FINAL FILES'
    ...
    def test_Descarga_archivos_MONTLHY_DC(self):
        ...
        #!!!!! Fecha de hoy !!!!!
        today = date.today()
        #today = datetime.date(2021, 9, 8) #Este solo lo usamos si queremos modificar la fecha en la que se esta corriendo el script formato ANO MES DIA
        #today_dayname = calendar.day_name[today.weekday()] #'Wednesday'
        ...
        if(num_unique_months==2):
            ...
            #tablevendor = 'vendor:vendorToggleImg'
            #self.driver.find_element_by_id(tablevendor).click()   
            ...
            Initial_path = self.dir_download 
            filename = max([Initial_path + "\\" + f for f in os.listdir(Initial_path)],key=os.path.getctime)
            new_name = 'SECOND SUPER VALU MONTLHY DC ' + 'week ' + str(week_number) + '.csv'
            shutil.move(filename,os.path.join(Initial_path,r'%s' %new_name))
            ...
            Initial_path = self.dir_download 
            filename = max([Initial_path + "\\" + f for f in os.listdir(Initial_path)],key=os.path.getctime)
            new_name = 'SUPER VALU MONTLHY DC ' + 'week ' + str(week_number) + '.csv'
            shutil.move(filename,os.path.join(Initial_path,r'%s' %new_name))
        ...
        self.PageProcess.macros_SV_Harbor(num_unique_months, self.import_path, self.export_path, week_number)
    ...
    def test_VOID(self):
        ...
        #MOVER EL ARCHIVO A LA UBICACION DESEADA
        new_download = self.export_Monthly_VOID
        shutil.move('%s'%self.dir_download+'\\%s'%new_name, '%s'%new_download+'\\%s'%new_name)
    ...
    def test_Melitta(self):
        ...
        #Seleccionamos el retailer deseado
        retailers = ['460865'] #' Montlhy DC' # 38811 - Plb Sports Inc' #'all' #or ' 24158 - American Licorice Co' or 'all'
        ...
        self.PageProcess.set_vendor(retailers,1)
        self.PageProcess.set_date_range(fecha_inicio_format,fecha_fin_format)
        self.PageProcess.set_Item_Code()
        self.PageProcess.download_page()
        time.sleep(5)
        ...
        Initial_path = self.dir_download 
        filename = max([Initial_path + "\\" + f for f in os.listdir(Initial_path)],key=os.path.getctime)
        new_name = 'MELITTA SUPER VALU week ' + str(week_number) + '.csv'
        shutil.move(filename,os.path.join(Initial_path,r'%s' %new_name))
        
        #MOVER EL ARCHIVO A LA UBICACION DESEADA
        new_download = self.export_Melitta
        shutil.move('%s'%self.dir_download+'\\%s'%new_name, '%s'%new_download+'\\%s'%new_name)
        ...
        print("SV MELITTA monthly is READY!!!!") 
        time.sleep(3)
